...
- `templates`: Portable copies of the gate, rules, skills, and rollout docs.
...
- Money math that uses float, binary rounding, or unquantized totals.
- UI code importing or duplicating `invoice_engine` behavior.
- API responses that stop returning decimal strings for money fields.
- Tests that mirror implementation instead of asserting expected behavior.
- Missing e2e coverage for visible invoice total changes.
